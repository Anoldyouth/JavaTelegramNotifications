...
public class UntrackShowMoreActionUnitTest {
    Action action;
    ...
    void prepare() {
        ...
        action = new UntrackShowMoreAction(scrapperClientMock, config);
    }

    @Test
    void emptyLinksList() {
        var update = ActionHelper.makeMockUpdateCallBackQuery("offset_0");
    ...
    @Test
    void severalPages() {
        var update = ActionHelper.makeMockUpdateCallBackQuery("offset_1");
        var searchResponse = mock(SearchLinksResponse.class);
        ...
        when(pagination.total()).thenReturn(3L);
        ...
        assertThat(((InlineKeyboardMarkup) response.request().getParameters().get("reply_markup"))
                .inlineKeyboard().length).isEqualTo(3);
    }
