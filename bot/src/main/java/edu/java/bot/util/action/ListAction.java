...
import edu.java.bot.util.response.list.ListResponse;
...
public class ListAction extends AbstractAction {
    ...
    protected Optional<ResponseData> process(Update update) {
        ...
        var replaceStateRequest = new ReplaceTgChatStateRequest(ScenarioDispatcher.ScenarioType.LIST);
        scrapperClient.replaceTgChatState(chatId, replaceStateRequest);

        if (response.pagination().total() > linksListConfig.limit()) {
            return Optional.of(new ListResponse(
                    chatId,
                    response.links(),
                    false,
                    null,
